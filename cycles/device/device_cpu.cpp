...
//    printf("Finalize over !\n");
                                 ...
                                 DenoisingTask *task)
  {
    ...
    float *master_weightAccum=(float*)malloc(64*sizeof(float) * w * h);
    float *master_out=(float*)malloc(64*sizeof(float) * w * h);
    memset(master_weightAccum, 0, sizeof(float) * w * h*64);
    memset(master_out, 0, sizeof(float) * w * h*64);

 /*   float *master_weightAccum=(float*)malloc(64*sizeof(float) * 10000);
    float *master_out=(float*)malloc(64*sizeof(float) * 10000);
    memset(master_weightAccum, 0, sizeof(float) * 10000*64);
    memset(master_out, 0, sizeof(float) *10000*64);
*/    typedef struct Para_lm{
    ...
    master_lm.w=w;    
    master_lm.h=h;    
    master_lm.guide_p=(float *)guide_ptr;    
    master_lm.variance_p=(float *)variance_ptr;    
    master_lm.image_p=(float *)image_ptr;    
    master_lm.weightAccum_p=(float *)master_weightAccum;    
    master_lm.out_p=(float *)master_out;    
    master_lm.stride=stride;    
    master_lm.channel_offset=channel_offset;    
    master_lm.k_2=k_2;    
    master_lm.a=a;


     __real_athread_spawn((void *)slave_func_lm,&master_lm,1);
     athread_join();
     float *op=(float *)out_ptr;
       
    for(int i=0;i<64;i++)
    {
        for(int j=0;j<w*h;j++){
           weightAccum[j]+=master_weightAccum[i*w*h+j];
           op[j]+=master_out[i*w*h+j];
        }
    }
...
*/
  ...
  bool denoising_construct_transform(DenoisingTask *task)
  {
    ...
    //printf("area.w=%d.area.z=%d\n",task->filter_area.w,task->filter_area.z);64,64
...
*/    
    ...
    float master_XtWX[4096*78*64]={0.0f};
    float3 master_XtWY[4096*12*64]={0.0f};
    //float master_XtWX[3840*78*64]={0.0f};
    //float3 master_XtWY[3840*12*64]={0.0f};
    ...
    Para_den master_den;
    master_den.r = task->radius;
    master_den.task_rs_source_w=task->reconstruction_state.source_w;
    master_den.task_rs_source_h=task->reconstruction_state.source_h;
    master_den.color_p=(float *)color_ptr;
    master_den.color_var_p=(float *)color_variance_ptr;
    master_den.scale_p=(float *)scale_ptr;
    master_den.stride=task->buffer.stride;
    master_den.channel_offset=task->buffer.pass_stride;
    master_den.frame_offset=frame_offset;
    master_den.k_2=task->nlm_k_2;
    master_den.t=task->tile_info->frames[frame];
    master_den.buffer=(float *)task->buffer.mem.device_pointer;
    master_den.transform=(float *)task->storage.transform.device_pointer;
    master_den.rank=(int *)task->storage.rank.device_pointer;
    master_den.XtWX=(float *)master_XtWX;
    master_den.XtWY=(float3 *)master_XtWY;
    master_den.filter_window=&task->reconstruction_state.filter_window.x;
    master_den.use_time=task->buffer.use_time;

     __real_athread_spawn((void *)slave_func1,&master_den,1);
     athread_join();

     float *X_ptr=(float *)task->storage.XtWX.device_pointer;
     float *Y_ptr=(float *)task->storage.XtWY.device_pointer;

     __real_athread_spawn((void *)slave_func_reduce,&master_den,1);
     athread_join();

     //memcpy(X_ptr,master_XtWX,3840*78*4);
     //memcpy(Y_ptr,master_XtWY,3840*12*4*4);
     memcpy(X_ptr,master_XtWX,4096*78*4);
     memcpy(Y_ptr,master_XtWY,4096*12*4*4);
  ...
  void path_trace(DeviceTask &task, RenderTile &tile, KernelGlobals *kg)
  {
    ...
    int average = tile.w*tile.h/384;
    int remainder = tile.h*tile.w%384;
    int mpi_offset[numprocs+1];
    int block = kernel_data.film.pass_stride;
    for(int i = 0; i < numprocs+1;++i){
        if(i*64<remainder){
            mpi_offset[i] = i*64*(average+1)*block;
        }
        else{
            mpi_offset[i] = (i*64*average + remainder)*block;
        }
    }

    float *render_buffer = (float *)tile.buffer;
//    master.buffer = (float *)tile.buffer;
    master.kg=kg;
//    float *masterbuffer = (float*)malloc(tile.h*tile.w*kernel_data.film.pass_stride*sizeof(float));
    master.buffer = render_buffer;
    ...
    for (int sample = start_sample; sample < end_sample; sample++) {
      ...
      master.h=tile.h;
      master.w=tile.w;
      master.offset=tile.offset;
      master.stride=tile.stride;


      __real_athread_spawn((void *)slave_func,&master,1);
      athread_join();
    ...
    //for (int sample = start_sample; sample < end_sample; sample++) {
      ...
      //master.sample=sample;
     /* master.proc_id = proc_id;
      master.sample=start_sample;
      master.x=tile.x;
      master.y=tile.y;
      master.h=tile.h;
      master.w=tile.w;
      master.offset=tile.offset;
      master.stride=tile.stride;
      master.start_sample=start_sample;
      master.end_sample=end_sample;

      __real_athread_spawn((void *)slave_func,&master,1);//flush
      athread_join();*/
//      MPI_Barrier(MPI_COMM_WORLD);
////////////////////////////////////////////////////
//      for(int i = 0 ;i < numprocs;++i){
//            MPI_Bcast(master.buffer+mpi_offset[i],mpi_offset[i+1]-mpi_offset[i],MPI_FLOAT,i,MPI_COMM_WORLD);
//       }
//      printf("proc_id %d ,start send recv\n",proc_id);
////////////////////////////////////////////////
//      if(proc_id == 0) {
//        for(int i = 1 ;i < numprocs;++i){
//            MPI_Recv(master.buffer+mpi_offset[i],mpi_offset[i+1]-mpi_offset[i],MPI_FLOAT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
//        }
//      }
//      else {
//
//            MPI_Send(master.buffer+mpi_offset[proc_id],mpi_offset[proc_id+1]-mpi_offset[proc_id],MPI_FLOAT,0,0,MPI_COMM_WORLD);
//      }
//////////////////////////////////////////////////////////////////////////////
     /* tile.sample = end_sample;*/

//    task.update_progress(&tile, tile.w * tile.h*tile.num_samples);
    if (use_coverage) {
      coverage.finalize();
    }
  ...
  void path_trace_skip(DeviceTask &task, RenderTile &tile, KernelGlobals *kg)
    {
      const bool use_coverage = kernel_data.film.cryptomatte_passes & CRYPT_ACCURATE;
  ...
  void denoise_skip(DenoisingTask &denoising, RenderTile &tile)
  {
    ProfilingHelper profiling(denoising.profiler, PROFILING_DENOISING);
    ...
    while (task.acquire_tile(this, tile, task.tile_types)) {
       if (tile.task == RenderTile::PATH_TRACE) {
        if (use_split_kernel) {
          ...
          float* render_buffer = (float *) tile.buffer;
        //  if(myid==0)  path_trace(task, tile, kg);
          if(trace_num%numprocs==myid ){
            path_trace(task, tile, kg);
            if(myid != 0)
                MPI_Send(render_buffer,tile.h*tile.w*kernel_data.film.pass_stride,MPI_FLOAT,0,0,MPI_COMM_WORLD);
          }
          else {
            path_trace_skip(task, tile, kg);
             if(myid == 0)
                 MPI_Recv(render_buffer,tile.h*tile.w*kernel_data.film.pass_stride,MPI_FLOAT,trace_num%numprocs,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
          }
        }
        trace_num++;
      }
        ...
        float* render_buffer = (float *) tile.buffer;
        //if(myid==0)   denoise(denoising, tile);
        if(denoise_num%numprocs==myid){
        ...
        {
           denoise_skip(denoising, tile);
           if(myid == 0)
               MPI_Recv(render_buffer,tile.h*tile.w*kernel_data.film.pass_stride,MPI_FLOAT,denoise_num%numprocs,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        }
        denoise_num++;
