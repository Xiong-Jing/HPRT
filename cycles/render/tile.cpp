...
bool TileManager::next_tile(Tile *&tile, int device, uint tile_types)
{
    ...
    int tile_index2 = -1;
    int logical_device2 = preserve_device ? device : 0;
    ...
    if(tile_index<0) 
    while (logical_device2 < state.denoising_tiles.size()) {
      if (state.denoising_tiles[logical_device2].empty()) {
        if (preserve_device) {
          break;
        }
        else {
      ...
      state.denoising_tiles[logical_device2].sort();
      tile_index2 = state.denoising_tiles[logical_device2].front();
      state.denoising_tiles[logical_device2].pop_front();
      break;
    }
    ...
    else if (tile_index2 >= 0) {
      tile = &state.tiles[tile_index2];
      return true;
    }
...
/*bool TileManager::next_tile_1(Tile *&tile, int device, uint tile_types)
{
  const bool preserve_device = preserve_tile_device && device < num_devices;
